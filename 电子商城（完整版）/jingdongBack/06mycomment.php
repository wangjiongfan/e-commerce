<?php
	...
	$uid = isset($_REQUEST['usId']) ? $_REQUEST['usId'] : '';
	// echo $uname;
	...
	$sql = "SELECT comment.Id,comment.pid,comment.uid,comment.commentary,comment.reply,comment.replayId,comment.replayTid,product.name as pname,user.name as commentUser FROM `comment` inner join `product` on comment.pid=product.Id LEFT join `user` on comment.replayId=user.id WHERE comment.uid=".$uid;
